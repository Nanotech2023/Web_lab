...
const defaultParams = `appid=${apiKey}&units=metric&lang=en`
...
app.get('/favourites', (req, res) => {
    ...
    db.collection('cities').find({}).toArray((err, items) => {
        ...
        if (!err) {
            ...
            let error = 'Ошибка в базе данных'
...
app.delete('/favourites', (req, res) => {
    ...
    db.collection('cities').find({}).toArray((err, items) => {
        ...
        db.collection('cities').deleteOne(details, (err, item) => {
            ...
            if (item.deletedCount === 0) err = "Данный город не был добавлен в избранное"
            formRes(res, err, len);
        });
    });
});
